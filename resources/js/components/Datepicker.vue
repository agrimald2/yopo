...
export default {
  ...
  data() {
    return {
      ...
      tmpMonth: moment().tz('America/Lima').month(),
      tmpYear: moment().tz('America/Lima').year(),
    }
  },
  computed: {
    year: {
    ...
    month: {
      get() {
        return moment(this.value || '').month()
      },
      set(value) {
        this.tmpMonth = value;
      }
    ...
    formatDateInput() {
      if (this.pickedDate) {
        ...
        return `${this.months[date.month()]} ${date.date()}, ${date.year()}`;
      }
      return null;
    },
    daysInMonth() {
      return moment().set({'year':this.tmpYear, 'month':this.tmpMonth}).daysInMonth();
    },
  },
  methods: {
    start() {
    ...
    pickDate(day) {
      var date = moment(this.value);
      // console.log(date);
      date.year(this.tmpYear);
      date.month(this.tmpMonth);
      date.date(day);
  ...
  watch: {
    value() {
      if (!this.value) {
        ...
        this.pickedDate = moment(this.value).date();
      }
    }
  }
}
